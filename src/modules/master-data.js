  ...
  const employeeClayDetail = {
    async render(main, params, signal) {
      ...
      const loaders = {
        ...
        payroll: async () => { const banks = (await api(`${B}/bank-accounts`)).items || []; return `<section class="mk-surface">${clayHead('card', 'Payroll & Bank', 'Komponen gaji dan rekening penggajian.', '')}<div class="mk-section-body mk-col"><div class="mk-g mk-g4"><div class="mk-inset mk-field"><label>Gaji Pokok</label><div class="mk-v">${money(comp.baseSalary || 0)}</div></div><div class="mk-inset mk-field"><label>Tunjangan Tetap</label><div class="mk-v">${money(comp.fixedAllowance || 0)}</div></div><div class="mk-inset mk-field"><label>Tunjangan Variabel</label><div class="mk-v">${money(comp.variableAllowance || 0)}</div></div><div class="mk-inset mk-field"><label>Total / Bulan</label><div class="mk-v mk-bl">${money(compTotal)}</div></div></div><div class="mk-tl-wrap"><div class="mk-o-caps">Rekening Penggajian</div>${banks.length ? `<div class="mk-g mk-g2">${banks.map((b) => `<div class="mk-bank"><div class="mk-bank-brand">${esc(b.bankName || '—')}</div><div class="mk-bank-no">${esc(b.accountNumber || '—')}</div><div class="mk-bank-foot"><span>${esc(b.accountHolder || ov.name || '')}</span>${chip(b.verificationStatus || 'PENDING')}</div></div>`).join('')}</div>` : emptyBox('Belum ada rekening payroll terdaftar.')}</div></div></section>`; },
