    ...
    async render(main, params, signal) {
      ...
      const loaders = {
        ...
        payroll: async () => {
          ...
          return `<section class="mk-surface">${clayHead('card', 'Payroll, Kompensasi & Bank', 'Komponen gaji, analisis grade band (compa-ratio), dan rekening penggajian.', '')}<div class="mk-section-body mk-col"><div class="mk-g mk-g4"><div class="mk-inset mk-field"><label>Gaji Pokok</label><div class="mk-v">${money(comp.baseSalary || 0)}</div></div><div class="mk-inset mk-field"><label>Tunjangan Tetap</label><div class="mk-v">${money(comp.fixedAllowance || 0)}</div></div><div class="mk-inset mk-field"><label>Tunjangan Variabel</label><div class="mk-v">${money(comp.variableAllowance || 0)}</div></div><div class="mk-inset mk-field"><label>Total / Bulan</label><div class="mk-v mk-bl">${money(compTotal)}</div></div></div>${bandCard}<div class="mk-tl-wrap"><div class="mk-o-caps">Slip Gaji / Payslip — Generator</div><div class="mk-inset mk-slip-gen"><div class="mk-g mk-g4"><div class="mk-field"><label>Periode</label><input type="month" class="mk-input" id="mkSlipPeriod" value="${new Date().toISOString().slice(0, 7)}"></div><div class="mk-field"><label>Bonus / THR</label><input type="number" class="mk-input" id="mkSlipBonus" value="0" min="0" step="100000"></div><div class="mk-field"><label>Status PTKP</label><select class="mk-input" id="mkSlipPtkp">${PTKP_OPTS.map((p) => `<option>${p}</option>`).join('')}</select></div><div class="mk-field"><label>Status NPWP</label><select class="mk-input" id="mkSlipNpwp"><option value="1">Ber-NPWP</option><option value="0">Tanpa NPWP (+20%)</option></select></div></div><button class="mk-btn primary" id="mkSlipGen">${MK('printer')} Generate Slip Gaji</button></div><div id="mkSlipOut"></div></div><div class="mk-tl-wrap"><div class="mk-o-caps">Rekening Penggajian</div>${banks.length ? `<div class="mk-g mk-g2">${banks.map((b) => `<div class="mk-bank"><div class="mk-bank-brand">${esc(b.bankName || '—')}</div><div class="mk-bank-no">${esc(b.accountNumber || '—')}</div><div class="mk-bank-foot"><span>${esc(b.accountHolder || ov.name || '')}</span>${chip(b.verificationStatus || 'PENDING')}</div></div>`).join('')}</div>` : emptyBox('Belum ada rekening payroll terdaftar.')}</div></div></section>`;
        },
        documents: async () => { const docs = (await api(`${B}/documents`)).items || []; return `<section class="mk-surface">${clayHead('fileText', 'Documents & Sertifikat', 'Dokumen kepegawaian dan sertifikasi.', `<span class="mk-badge slate">${docs.length} dokumen</span>`)}<div class="mk-section-body">${docs.length ? `<div class="mk-g mk-g3">${docs.map((d) => `<div class="mk-inset mk-doc"><span class="mk-doc-ic">${MK('fileText')}</span><div class="mk-flex1"><b>${esc(d.title || d.documentType || 'Dokumen')}</b><small>${esc(d.documentType || '')}${d.expiryDate ? ` · exp ${fmtDate(d.expiryDate)}` : ''}</small></div>${(d.verified || d.verificationStatus === 'VERIFIED') ? '<span class="mk-badge emerald">' + MK('check') + '</span>' : '<span class="mk-badge amber">•</span>'}</div>`).join('')}</div>` : emptyBox('Belum ada dokumen diunggah.')}</div></section>`; },
