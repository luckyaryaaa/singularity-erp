      ...
      const renderTab = async (tabId) => {
        ...
        if (tabId === 'overview') {
          if (params.type === 'employees') {
            const s=overview.enterpriseSummary||{},pos=s.currentPosition||{},employment=s.employment||{},comp=s.compensation||{},tax=s.tax||{},bank=s.payrollBank||{},sup=s.supervisor||{};
            ...
            body.innerHTML=`${employeeQualityPanel(overview)}
              ...
              <section class="dashboard-grid"><article class="panel"><header><div><p class="eyebrow">EMPLOYMENT & ORGANISASI</p><h2>Penempatan & garis komando</h2></div>${chip(employment.employmentStatus||overview.lifecycleStatus||'ACTIVE')}</header><div class="panel-body"><dl class="detail-dl"><div><dt>Jabatan</dt><dd>${esc(pos.positionTitle||overview.jobTitle||'—')}</dd></div><div><dt>Divisi / departemen</dt><dd>${esc(pos.division||overview.department||'—')}</dd></div><div><dt>Lokasi kerja</dt><dd>${esc(pos.workLocation||overview.branchName||'—')}</dd></div><div><dt>Grade</dt><dd>${esc(pos.salaryGrade||comp.salaryGrade||'—')}</dd></div><div><dt>Tipe & status</dt><dd>${esc([employment.employmentType,employment.employmentStatus].filter(Boolean).join(' · ')||'—')}</dd></div><div><dt>Masa kerja</dt><dd>${serviceLength(overview.joinDate)}${overview.joinDate?` · sejak ${fmtDate(overview.joinDate)}`:''}</dd></div><div><dt>Atasan langsung</dt><dd>${sup.supervisorName?`${esc(sup.supervisorName)}${sup.supervisorTitle?` · ${esc(sup.supervisorTitle)}`:''}`:'<span class="muted">Belum ditetapkan</span>'}</dd></div><div><dt>Bawahan langsung</dt><dd>${Number(s.directReports||0)} orang</dd></div></dl></div></article>
              <article class="panel"><header><div><p class="eyebrow">BENEFIT & KEPATUHAN</p><h2>Jaminan sosial & akses</h2></div></header><div class="panel-body stack"><div class="stat-row"><span>Program BPJS aktif</span><b>${Number(s.bpjsPrograms||0)}</b></div><div class="stat-row"><span>Polis asuransi aktif</span><b>${Number(s.insurancePolicies||0)}</b></div><div class="stat-row"><span>Rekening payroll</span><b>${esc(bank.bankName||'Belum ada')}${bank.accountNumber?` · ${esc(bank.accountNumber)}`:''}</b></div><div class="stat-row"><span>Dokumen ≤ 90 hari</span><b>${Number(s.expiringDocuments||0)}</b></div><div class="stat-row"><span>Akun sistem aktif</span><b>${Number(s.activeUserAccounts||0)}</b></div><div class="stat-row"><span>Kehadiran bulan ini</span><b>${Number(s.attendanceDays||0)} hari</b></div></div></article></section>
