...
test('PostgreSQL numbering concurrency: 24 transaksi menghasilkan nomor unik', async () => {
  ...
  try {
    const numbers=await Promise.all(Array.from({length:24},async()=>{
      ...
      try { await client.query('BEGIN'); await client.query("SELECT set_config('app.is_system','on',true),set_config('app.is_platform','on',true),set_config('app.tenant_id','00000000-0000-0000-0000-000000000001',true)"); const n=await runtime.nextNumber(client,{documentType:'INVOICE',branchId}); await client.query('COMMIT'); return n; }
...
test('PostgreSQL document transaction: audit+outbox atomic dan stale version ditolak', async()=>{
  ...
  try {
    const client=new Client({connectionString:process.env.DATABASE_URL}); await client.connect();
    try {
      await client.query('BEGIN'); await client.query("SELECT set_config('app.is_system','on',true),set_config('app.is_platform','on',true),set_config('app.tenant_id','00000000-0000-0000-0000-000000000001',true)"); doc=await runtime.createDocument(client,{type:'INVOICE',user,title:'Atomic Invoice',amount:1000}); await client.query('COMMIT');
      ...
      await client.query('BEGIN'); await client.query("SELECT set_config('app.is_system','on',true),set_config('app.is_platform','on',true),set_config('app.tenant_id','00000000-0000-0000-0000-000000000001',true)"); const updated=await runtime.updateDocument(client,{id:doc.id,expectedVersion:1,patch:{title:'v2'},user}); await client.query('COMMIT');
...
test('PostgreSQL idempotency: 12 request paralel mengeksekusi handler tepat sekali',async()=>{
  ...
  try{
    const results=await Promise.all(Array.from({length:12},async()=>{
      const client=new Client({connectionString:process.env.DATABASE_URL});await client.connect();
      try{await client.query('BEGIN'); await client.query("SELECT set_config('app.is_system','on',true),set_config('app.is_platform','on',true),set_config('app.tenant_id','00000000-0000-0000-0000-000000000001',true)");const result=await runtime.withIdempotency(client,{userId,operation:'invoice.issue',key,body:{amount:5000}},async()=>{executions++;return{status:201,body:{documentId:'one'}};});await client.query('COMMIT');return result;}
      catch(error){await client.query('ROLLBACK');throw error;}finally{await client.end();}
    }));
    assert.equal(executions,1);assert.equal(results.filter(r=>r.body.idempotentReplay).length,11);
    const client=new Client({connectionString:process.env.DATABASE_URL});await client.connect();await client.query('BEGIN'); await client.query("SELECT set_config('app.is_system','on',true),set_config('app.is_platform','on',true),set_config('app.tenant_id','00000000-0000-0000-0000-000000000001',true)");
...
test('PostgreSQL approval: jenjang supervisor → finance → owner tidak dapat dilompati',async()=>{
  ...
  const client=new Client({connectionString:process.env.DATABASE_URL});await client.connect();
  try{
    await client.query('BEGIN'); await client.query("SELECT set_config('app.is_system','on',true),set_config('app.is_platform','on',true),set_config('app.tenant_id','00000000-0000-0000-0000-000000000001',true)");doc=await runtime.createDocument(client,{type:'PURCHASE_ORDER',user:users.sales,title:'PO besar',amount:75_000_000});await runtime.transitionDocument(client,{id:doc.id,action:'submit',user:users.sales});await client.query('COMMIT');
    await client.query('BEGIN'); await client.query("SELECT set_config('app.is_system','on',true),set_config('app.is_platform','on',true),set_config('app.tenant_id','00000000-0000-0000-0000-000000000001',true)");await assert.rejects(()=>runtime.transitionDocument(client,{id:doc.id,action:'approve',user:users.owner}),e=>e.code==='PERMISSION_DENIED');await client.query('ROLLBACK');
    for(const role of ['hrd','finance_manager','owner']){await client.query('BEGIN'); await client.query("SELECT set_config('app.is_system','on',true),set_config('app.is_platform','on',true),set_config('app.tenant_id','00000000-0000-0000-0000-000000000001',true)");doc=await runtime.transitionDocument(client,{id:doc.id,action:'approve',user:users[role]});await client.query('COMMIT');}
