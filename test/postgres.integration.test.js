...
pgTest('PostgreSQL integration: document lines, inventory ledger, dan jurnal double-entry atomic', async()=>{
  ...
  }finally{for(const id of ids.docs){await admin.query('DELETE FROM journal_lines WHERE journal_document_id=$1',[id]).catch(()=>{});await admin.query('DELETE FROM inventory_movements WHERE document_id=$1',[id]).catch(()=>{});await admin.query('DELETE FROM document_postings WHERE document_id=$1',[id]).catch(()=>{});await admin.query('DELETE FROM document_lines WHERE document_id=$1',[id]).catch(()=>{});await admin.query('DELETE FROM audit_logs WHERE entity_id=$1',[id]).catch(()=>{});await admin.query("DELETE FROM domain_event_outbox WHERE payload->>'entityId'=(SELECT document_number FROM business_documents WHERE id=$1)",[id]).catch(()=>{});await admin.query('DELETE FROM business_documents WHERE id=$1',[id]).catch(()=>{});}if(ids.productId){await admin.query('DELETE FROM inventory_balances WHERE product_id=$1',[ids.productId]).catch(()=>{});await admin.query('DELETE FROM products WHERE id=$1',[ids.productId]).catch(()=>{});}await Promise.all([client.end(),admin.end()]);}
});
...
pgTest('PostgreSQL Sprint 3: conversion, master CRUD, private file, artifact, dan matriks role',async()=>{
  ...
  try{await client.query('BEGIN');const user=(await client.query(`SELECT id,username,display_name "displayName",role,branch_id "branchId",branch_scope "branchScope" FROM app_users WHERE role='owner' AND active LIMIT 1`)).rows[0];assert.ok(user);
    ...
    const uploaded=await privateStorage.upload(client,{buffer:Buffer.from('%PDF-1.4\n%%EOF'),filename:'sprint-3.pdf',mimeType:'application/pdf',user,module:'quotation',documentId:source.id});files.push(path.join(privateStorage.ROOT,uploaded.storagePath));const downloaded=await privateStorage.download(client,uploaded.id);assert.equal(downloaded.buffer.toString(),'%PDF-1.4\n%%EOF');
