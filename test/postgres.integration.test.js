...
pgTest('PostgreSQL integration: document lines, inventory ledger, dan jurnal double-entry atomic', async()=>{
  ...
  const client=new Client({connectionString:process.env.DATABASE_URL}),admin=new Client({connectionString:process.env.MIGRATION_DATABASE_URL});await Promise.all([client.connect(),admin.connect()]); await client.query("SELECT set_config('app.is_system','on',false),set_config('app.is_platform','on',false),set_config('app.tenant_id','00000000-0000-0000-0000-000000000001',false)");const ids={docs:[]};
...
pgTest('PostgreSQL Sprint 3: conversion, master CRUD, private file, artifact, dan matriks role',async()=>{
  ...
  try{await client.query('BEGIN'); await client.query("SELECT set_config('app.is_system','on',true),set_config('app.is_platform','on',true),set_config('app.tenant_id','00000000-0000-0000-0000-000000000001',true)");const user=(await client.query(`SELECT id,username,display_name "displayName",role,branch_id "branchId",branch_scope "branchScope" FROM app_users WHERE role='owner' AND active LIMIT 1`)).rows[0];assert.ok(user);
    ...
    const roleUsers=[['admin','system_admin'],['finance','finance_manager'],['accounting','accounting'],['tax','tax'],['hrd','hrd'],['sales','sales'],['procurement','procurement'],['warehouse','warehouse'],['production','production'],['employee','employee']],expectedRoles=roleUsers.map(x=>x[1]),roles=(await client.query(`SELECT DISTINCT role FROM app_users WHERE role=ANY($1::text[])`,[expectedRoles])).rows.map(row=>row.role);assert.deepEqual(new Set(roles),new Set(expectedRoles));
    // Pulihkan baseline UAT akun peran DI DALAM transaksi ini saja: aktivitas UAT nyata
    // (mis. system_admin 'admin' pada insiden SEC-UAT-001) menggeser password/must_change/lock.
    // ROLLBACK di akhir uji memastikan akun UAT langsung milik pengguna tidak berubah permanen.
    // Tanpa ini uji rapuh terhadap drift keadaan basis data dev/UAT yang dipakai bersama.
    await client.query(`UPDATE app_users SET password_hash=$2,must_change_password=true,failed_login_count=0,locked_until=NULL,active=true WHERE username=ANY($1::text[])`,[roleUsers.map(x=>x[0]),hashPassword(process.env.MAT_UAT_DEFAULT_PASSWORD)]);
