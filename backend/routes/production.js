...
async function dispatch(client, req, pathname, ctx) {
  ...
  if (method === 'GET' && match) { assertPermission(ctx.user, 'production.view'); return { body: await production.productionPanel(client, match[1], ctx.user) }; }
  ...
  if (method === 'GET' && match) { assertPermission(ctx.user, 'quality.view'); return { body: await production.listInspections(client, match[1], ctx.user) }; }
  ...
  if (method === 'GET' && pathname === '/api/production/stock-locations') { assertPermission(ctx.user, 'production.view'); return { body: await production.listStockLocations(client, ctx.user) }; }
  if (method === 'GET' && pathname === '/api/production/work-centers') {
    ...
    return { body: { items: (await client.query(`SELECT wc.id,wc.code,wc.name,wc.work_center_type,wc.capacity_hours_per_day,wc.hourly_rate,p.name plant_name FROM work_centers wc JOIN plants p ON p.id=wc.plant_id WHERE wc.active AND ($1 OR p.branch_id=$2) ORDER BY wc.code`, [scopeAll, ctx.user.branchId])).rows.map(runtime.camel) } };
  }
  ...
  if (method === 'GET' && pathname === '/api/mrp/suggestions') { assertPermission(ctx.user, 'production.view'); return { body: await production.listMrp(client) }; }
