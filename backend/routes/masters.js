...
async function dispatch(client, req, url, ctx) {
  ...
  m=p.match(/^\/api\/masters\/customers\/([0-9a-f-]{36})\/commercial$/);
  if(method==='GET'&&m){return masterData.customerCommercial(client,m[1],ctx.user);}
  ...
  m=p.match(/^\/api\/masters\/employees\/([0-9a-f-]{36})\/offboarding$/);
  if(method==='GET'&&m)return masterData.getOffboarding(client,m[1],ctx.user);
  if(method==='POST'&&m){const body=await readBody(req);ctx.status=201;return masterData.initiateOffboarding(client,m[1],body,ctx.user,ctx.requestId);}
  m=p.match(/^\/api\/masters\/employees\/([0-9a-f-]{36})\/offboarding\/([0-9a-f-]{36})\/clearance$/);
