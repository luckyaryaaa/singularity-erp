...
async function dispatch(client, req, url, ctx) {
  ...
  if(method==='POST'&&p==='/api/hr/attendance/scan'){assertPermission(ctx.user,'attendance.edit');const body=await readBody(req);const result=await businessOps.scanAttendance(client,{code:body.code,user:ctx.user});await runtime.audit(client,{userId:ctx.user.id,action:'SCAN_ATTENDANCE',module:'attendance',entityType:'ATTENDANCE',entityId:result.employeeId,newValue:{action:result.action,code:result.employeeCode},requestId:ctx.requestId,branchId:ctx.user.branchId});ctx.status=201;return result;}
