...
async function runBackup(client,{requestedBy=null}={}){
  ...
  try{const pgDump=await executable('pg_dump');await runFile(pgDump,['--format=custom','--compress=9','--no-owner','--no-acl','--host',cfg.host,'--port',cfg.port,'--username',cfg.user,'--file',file,cfg.database],{env:{...process.env,PGPASSWORD:cfg.password},windowsHide:true,maxBuffer:1024*1024});const bytes=await fs.readFile(file),checksum=createHash('sha256').update(bytes).digest('hex'),sizeMb=Math.max(1,Math.ceil(bytes.length/1048576));
    let offsite={enabled:false};
    try{offsite=await offsiteCopy(file,bytes);}
    ...
    const target=offsite.enabled&&!offsite.error?'LOCAL+OFFSITE_ENCRYPTED':'LOCAL_PRIVATE';
  ...
  catch(error){await client.query(`UPDATE backup_runs SET status='FAILED',finished_at=now(),error=$2 WHERE id=$1`,[id,String(error.message).slice(0,2000)]);alerts.send('Backup database GAGAL',error.message,{key:'backup-run'}).catch(()=>{});throw error;}
}
...
module.exports={ROOT,runBackup,restoreDrill,prune,executable,offsiteCopy};
