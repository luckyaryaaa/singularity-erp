...
const MASTER={customers:{table:'customers',search:['code','name','city'],sort:new Set(['code','name','city','created_at'])},suppliers:{table:'suppliers',search:['code','name','category'],sort:new Set(['code','name','category','created_at'])},products:{table:'products',search:['code','name','uom'],sort:new Set(['code','name','created_at'])},employees:{table:'employees',search:['employee_code','nik','name','department','job_title'],sort:new Set(['employee_code','nik','name','department','join_date'])}};
const MASTER_FIELDS={
  customers:['code','name','customer_type','legal_name','npwp','ppn_status','business_category','city','address','website','payment_term_days','credit_limit_amount','risk_rating','customer_since','assigned_sales','currency','credit_hold','collection_status','tax_treatment','effective_from','effective_to','active'],
...
// C17 — kolom yang tidak boleh ikut pada daftar umum. `SELECT m.*` sebelumnya
// mengirim base_salary ke SETIAP pemegang employee.view (mis. supervisor
// operasional). Detail gaji tetap tersedia lewat modul payroll bagi yang
// berhak; daftar master bukan tempatnya.
const MASTER_SENSITIVE={employees:{base_salary:'payroll.view',bpjs:'payroll.view'}};
function redactMasterRow(name,row,user){
  const rules=MASTER_SENSITIVE[name];if(!rules)return row;
  const out={...row};
  for(const [column,permission] of Object.entries(rules)) if(!hasPermission(user,permission)) delete out[column];
  return out;
}
const snake=value=>value.replace(/[A-Z]/g,c=>`_${c.toLowerCase()}`);
// Kode Karyawan non-sensitif EMP-<TENANT>-<urutan> (prefix dari tenants.code).
// Urutan aman-balapan lewat UPSERT atomik pada penghitung per-tenant.
const CUR_TENANT=`COALESCE(nullif(current_setting('app.tenant_id',true),'')::uuid,'00000000-0000-0000-0000-000000000001'::uuid)`;
async function nextEmployeeCode(client){
  const t=(await client.query(`SELECT code FROM tenants WHERE id=${CUR_TENANT}`)).rows[0];
  const prefix=((t&&t.code?String(t.code):'tnt').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,8))||'TNT';
  const n=(await client.query(`INSERT INTO tenant_employee_seq(tenant_id,last_no) VALUES(${CUR_TENANT},1) ON CONFLICT(tenant_id) DO UPDATE SET last_no=tenant_employee_seq.last_no+1 RETURNING last_no`)).rows[0].last_no;
  return `EMP-${prefix}-${String(n).padStart(4,'0')}`;
}
async function createMaster(client,name,body,user){const fields=MASTER_FIELDS[name];if(!fields)throw new AppError('RESOURCE_NOT_FOUND');await masterGovernance.validateMaster(client,name,body);const entries=Object.entries(body||{}).map(([k,v])=>[snake(k),v]).filter(([k,v])=>fields.includes(k)&&v!==undefined&&v!=='');if(!entries.length)throw new AppError('VALIDATION_ERROR','Data master kosong.');const columns=['id',...entries.map(x=>x[0])],values=[randomUUID(),...entries.map(x=>x[1])];if(['customers','suppliers'].includes(name)){columns.push('created_by','updated_by');values.push(user.id,user.id);}if(name==='employees'){columns.push('employee_code');values.push(await nextEmployeeCode(client));}const params=values.map((_,i)=>`$${i+1}`);const row=(await client.query(`INSERT INTO ${name}(${columns.join(',')}) VALUES(${params.join(',')}) RETURNING *`,values)).rows[0];await masterGovernance.refreshQuality(client,name,row.id);return camel((await client.query(`SELECT * FROM ${name} WHERE id=$1`,[row.id])).rows[0]);}
...
async function updateMaster(client,name,id,body,user,requestId=null){
  ...
  if(!entries.length)throw new AppError('VALIDATION_ERROR','Tidak ada perubahan valid.');
  const current=(await client.query(`SELECT * FROM ${name} WHERE id=$1`,[id])).rows[0];
  if(!current)throw new AppError('RESOURCE_NOT_FOUND');

  const changeRequests=require('./change-requests');
  const {direct,controlled}=changeRequests.split(name,entries.map(([k,v])=>[k,v===''?null:v]),current);
  let changeRequest=null;
  if(Object.keys(controlled).length){
    changeRequest=await changeRequests.submit(client,{entityType:name,entityId:id,changes:controlled,
  ...
  if(direct.length){
    ...
    if(['customers','suppliers'].includes(name)){values.push(user.id);sets.push(`updated_by=$${values.length}`);}
    const row=(await client.query(`UPDATE ${name} SET ${sets.join(',')} WHERE id=$1 RETURNING *`,values)).rows[0];
    if(!row)throw new AppError('RESOURCE_NOT_FOUND');
  }
  await masterGovernance.refreshQuality(client,name,id);
  const saved=camel((await client.query(`SELECT * FROM ${name} WHERE id=$1`,[id])).rows[0]);
  // Kolom terkendali sengaja dikembalikan sebagai pendingChanges, bukan diam-diam
  // diabaikan — pemanggil harus tahu bahwa sebagiannya belum berlaku.
  return changeRequest?{...saved,pendingChanges:{requestId:changeRequest.id,fields:Object.keys(controlled),supersededCount:changeRequest.supersededCount}}:saved;
}
async function listMaster(client,name,query={},user=null){const cfg=MASTER[name];if(!cfg)throw new AppError('RESOURCE_NOT_FOUND');const limit=Math.min(Math.max(Number(query.limit)||25,1),100),page=Math.max(Number(query.page)||1,1),params=[];let where='TRUE';if(query.q){params.push(`%${String(query.q).slice(0,120)}%`);where='('+cfg.search.map(c=>`${c} ILIKE $1`).join(' OR ')+')';}const sort=cfg.sort.has(query.sortKey)?query.sortKey:name==='employees'?'name':'code',dir=query.sortDir==='desc'?'DESC':'ASC';params.push(limit,(page-1)*limit);const li=params.length-1,off=params.length;const from=name==='employees'?'employees m LEFT JOIN branches b ON b.id=m.branch_id':`${cfg.table} m`;const select=name==='employees'?'m.*,b.name branch_name':'m.*';const qualifiedWhere=where.replace(/\b(employee_code|code|name|city|category|uom|nik|department|job_title)\b/g,'m.$1');const items=(await client.query(`SELECT ${select} FROM ${from} WHERE ${qualifiedWhere} ORDER BY m.${sort} ${dir} LIMIT $${li} OFFSET $${off}`,params)).rows.map(r=>camel(redactMasterRow(name,r,user)));const total=Number((await client.query(`SELECT count(*) n FROM ${from} WHERE ${qualifiedWhere}`,query.q?[params[0]]:[])).rows[0].n);
...
async function listInventory(client,user,{page=1,limit=25,q,readGrain='BRANCH'}={}){const canonical=String(readGrain).toUpperCase()==='CANONICAL';limit=Math.min(Math.max(Number(limit)||25,1),100);page=Math.max(Number(page)||1,1);const params=[];let where='TRUE';if(!['owner','admin'].includes(user.role)&&user.branchScope!=='*'){params.push(user.branchId);where+=` AND i.warehouse_id=$${params.length}`;}if(q){params.push(`%${String(q).slice(0,120)}%`);where+=` AND (p.code ILIKE $${params.length} OR p.name ILIKE $${params.length})`;}const total=Number((await client.query(`SELECT count(*) n FROM inventory_balances i JOIN products p ON p.id=i.product_id WHERE ${where}`,params)).rows[0].n);params.push(limit,(page-1)*limit);const items=(await client.query(`SELECT i.*,p.code product_code,p.name product_name,p.uom,w.code warehouse_code,w.name warehouse_name FROM inventory_balances i JOIN products p ON p.id=i.product_id ${canonical?'JOIN org_warehouses w ON w.id=i.org_warehouse_id':'JOIN branches w ON w.id=i.warehouse_id'} WHERE ${where} ORDER BY p.code LIMIT $${params.length-1} OFFSET $${params.length}`,params)).rows.map(camel);return{items,page,limit,total,readGrain:canonical?'CANONICAL':'BRANCH',totalPages:Math.max(Math.ceil(total/limit),1)};}
