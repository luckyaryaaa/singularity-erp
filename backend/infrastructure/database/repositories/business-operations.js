...
// Absensi via scan QR/barcode kartu: resolve karyawan dari kode, lalu check-in
// (scan pertama hari itu) atau check-out (scan kedua). Sumber = QR_SCAN.
async function scanAttendance(client,{code,user}){
  const key=String(code||'').trim().toUpperCase();
  if(!key)throw new AppError('VALIDATION_ERROR','Kode kartu kosong.');
  const emp=(await client.query('SELECT id,name,branch_id,employee_code FROM employees WHERE active AND (upper(employee_code)=$1 OR nik=$1) LIMIT 1',[key])).rows[0];
  if(!emp)throw new AppError('RESOURCE_NOT_FOUND','Kartu tidak dikenali. Periksa kode karyawan.');
  const today=new Date().toISOString().slice(0,10),nowIso=new Date().toISOString();
  const existing=(await client.query('SELECT check_in,check_out FROM attendance_records WHERE employee_id=$1 AND work_date=$2',[emp.id,today])).rows[0];
  let action,checkIn,checkOut;
  if(!existing||!existing.check_in){action='CHECK_IN';checkIn=nowIso;checkOut=existing?existing.check_out:null;}
  else if(!existing.check_out){if(new Date(nowIso)<=new Date(existing.check_in)){action='ALREADY_IN';checkIn=existing.check_in;}else{action='CHECK_OUT';checkIn=existing.check_in;checkOut=nowIso;}}
  else{action='COMPLETE';checkIn=existing.check_in;checkOut=existing.check_out;}
  if(action==='CHECK_IN'||action==='CHECK_OUT')await upsertAttendance(client,{employeeId:emp.id,workDate:today,checkIn,checkOut,status:'PRESENT',source:'QR_SCAN',user});
  const hhmm=(v)=>v?new Date(v).toTimeString().slice(0,5):null;
  return {action,employeeId:emp.id,employeeName:emp.name,employeeCode:emp.employee_code,workDate:today,checkIn:hhmm(checkIn),checkOut:hhmm(checkOut)};
}
...
module.exports={period,accountingSummary,ledger,closePeriod,reopenPeriod,allocatePayment,reversePayment,attendance,upsertAttendance,scanAttendance,leaveBalances,createPayroll,payrollItems,payrollSelf,syncTaxes,taxSummary,reportTax,reconcile};
