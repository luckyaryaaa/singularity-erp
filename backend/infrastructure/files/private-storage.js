...
async function scopedMetadata(client,id,user){return camel((await client.query(`SELECT * FROM file_metadata WHERE id=$1 AND NOT is_deleted AND ($2::boolean OR branch_id=$3 OR uploaded_by=$4)`,[id,hasGlobalScope(user),user?.branchId||null,user?.id||null])).rows[0]);}
