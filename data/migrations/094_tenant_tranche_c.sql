...
-- 094 · Singularity Fase 0 — tranche C: tenant_id + RLS pada tabel BISNIS/CONFIG
-- yang BELUM ber-RLS (enable RLS pertama kali di sini).
--
-- DIKECUALIKAN (tetap GLOBAL, ditangani terpisah / memang lintas-tenant):
--   auth/identity (user_sessions, auth_pending, mfa_recovery_codes, webauthn_
--   credentials, password_reset_requests, login_history, user_role_assignments,
--   emergency_access_overrides), RBAC/governance (roles, enterprise_roles,
--   account_roles, role_permissions, permission_catalog, approval_matrix/
--   policy_versions, access_reviews[_items], sod_rules, sod_conflict_events),
--   platform infra (schema_migrations, field_encryption_rotations, background_
--   jobs, idempotency_records, backup_runs, domain_event_outbox, reporting_
--   refresh_runs, data_retention_*), dan reference global (currencies).
--   audit_logs* (partisi) = tranche audit terpisah.
--
-- Pola: tenant_id NOT NULL fallback-default (COALESCE→MAT #001), backfill MAT,
-- FK, index, ENABLE RLS, permissive baseline + RESTRICTIVE tenant policy.
